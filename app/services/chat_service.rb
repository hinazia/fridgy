class ChatService
  ...
  def build_prompt
    ...
    @prompt = "Hello ChatGPT!! Please, can you give me a recipe based on these ingredients, that is not the same as the last one, step by step, with each line numbered: #{ingredients} for a #{@meal_type} meal of the #{@category} cuisine. Please return a JSON with the information in the following format: {\"title\": \"Title of the Recipe\", \"content\": \"Instructions of the recipe\"}. Ensure the JSON is valid by escaping special characters and removing any newlines."
  end
